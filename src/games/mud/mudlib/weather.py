 ...
 */
 ...
 cancarry(plyr)
    ...
    a=0;
    b=0;
    if(PlayerCharacter.load(plyr).is_moderator) return(1);
    if(PlayerCharacter.load(plyr).is_special) return(1);
    while(a<numobs)
       {
       if((iscarrby(a,plyr))&&(!isdest(a))) b++;
       a++;
       }
    if(b<PlayerCharacter.load(plyr).level+5) return(1);
    return(0);
    }
 ...
 setcom()
    ...
    b=numarg(wordbuf);
    if(b>Item.load(a).max_state)
       {
       bprintf("Sorry max state for that is %d\n",Item.load(a).max_state);
       return;
       }
    if(b<0)
       {
...
bitset:if(brkword()==-1)
       {
       	...
       	if(brkword()==-1)
       	{
       	   bprintf("The bit is %s\n",Item.load(a).get_flag(b)?"TRUE":"FALSE");
       	   return;
       	}
       	c=numarg(wordbuf);
       	if((c<0)||(c>1)||(b<0)||(b>15))
...
setmobile:a=fpbn(wordbuf);
           ...
           b=numarg(wordbuf);
           PlayerCharacter.load(a).strength = b
    }
...
while(c<numobs)
{
if((c!=32)&&(Item.load(c).get_flag(Item.FLAG_13)==0)) {c++;continue;}
if(ishere(c)) return(0);
if((Item.load(c).is_in_channel)||(Item.load(c).is_in_item)) {c++;continue;}
if(PlayerCharacter.load(Item.load(c).location_id).channel_id!=curch) {c++;continue;}
return(0);
}
return(1);
}
