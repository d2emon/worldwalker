...
long it;
    ...
    switch(it)
       {
case -1:return(4);
default:if(!Item.load(it).is_weapon)return(-1);
else return(Item.load(it).get_value(0));
          }
...
void hitplayer(victim,wpn)
 long victim,wpn;
    ...
    long x[4];
    long cth,ddn,res;
    if(!strlen(PlayerCharacter.load(victim).name)) return;
    /* Chance to hit stuff */
	...
	if(cth>res)
        {
	       bprintf("You hit \001p%s\001 ",PlayerCharacter.load(victim).name);
 	       if(wpn!= -1)bprintf("with the %s",Item.load(wpn).name);
	       bprintf("\n");
	       ddn=randperc()%(dambyitem(wpn));
	       x[0]=mynum;
	       x[1]=ddn;
	       x[2]=wpn;
	       if(PlayerCharacter.load(victim).strength-ddn<0)
		{
			bprintf("Your last blow did the trick\n");
			if(not PlayerCharacter.load(victim).is_dead)
			{
/* Bonus ? */
				if(victim<16) my_sco+=(PlayerCharacter.load(victim)level*PlayerCharacter.load(victim).level*100);
				else my_sco+=10*damof(victim);
			}
			PlayerCharacter.load(victim).kill(); /* MARK ALREADY DEAD */
			in_fight=0;
			fighting= -1;
		}
       if(victim<16)sendsys(PlayerCharacter.load(victim).name,globme,-10021,curch,(char *)x);
       else
          	{
          	woundmn(victim,ddn);
          	}
       ...
       {
	       bprintf("You missed \001p%s\001\n",PlayerCharacter.load(victim).name);
	       x[0]=mynum;
	       x[1]= -1;
	       x[2]=wpn;
	       if(victim<16) sendsys(PlayerCharacter.load(victim).name,globme,-10021,curch,(char *)x);
...
void  bloodrcv(array,isme)
 long *array;
    ...
    if(array[0]<0) return;
    nlod:if(!PlayerCharacter.load(array[0]).exists) return;
    fighting=array[0];
    in_fight=300;
    if(array[1]== -1)
       {
       bprintf("\001p%s\001 attacks you",PlayerCharacter.load(array[0]).name);
       if(array[2]!= -1)bprintf(" with the %s",Item.load(array[2]).name);
       bprintf("\n");
       }
    else
       {
       bprintf("You are wounded by \001p%s\001",PlayerCharacter.load(array[0]).name);
       if(array[2]>-1)bprintf(" with the %s",Item.load(array[2]).name);
       bprintf("\n");
    ...
    if(my_str<0)
    {
          logging("%s slain by %s",globme,PlayerCharacter.load(array[0]).name);
          dumpitems();
          loseme();
          closeworld();
          delpers(globme);
