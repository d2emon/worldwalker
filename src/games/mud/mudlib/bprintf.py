...
if(strlen(x)>235)
{
logging("Bprintf Short Buffer overflow");
...
int seeplayer(x)
    ...
    if(not PlayerCharacter.load(x).visible_for(PlayerCharacter.load(mynum).level)) return(0);
    if(ail_blind) return(0); /* Cant see */
    if((curch==PlayerCharacter.load(x).location_id)&&(isdark(curch)))return(0);
    setname(x);
    return(1);
    }
...
void pbfr()
    ...
    if(snoopd!=-1)
       {
       fln=opensnoop(PlayerCharacter.load(snoopd).name,"a");
       if(fln>0)
          {
iskb=0;
          dcprnt(sysbuf,fln);
...
void quprnt(x)
 char *x;
    ...
    if((strlen(x)+strlen(sysbuf))>4095)
       ...
       strcpy(sysbuf,"");
       loseme();
       logging("Buffer overflow on user %s",globme);
...
void snoopcom()
    ...
    if(((my_lev<10000)&&(PlayerCharacter.load(x).is_moderator))||(ptstbit(x,6)))
       {
       bprintf("Your magical vision is obscured\n");
       snoopt= -1;
       return;
       }
    strcpy(sntn,PlayerCharacter.load(x).name);
    snoopt=x;
...
long x;
{
	...
	{
		strcpy(wd_it,PlayerCharacter.load(x).name);
		return;
	}
	if(PlayerCharacter.load(x).sex == PlayerCharacter.FEMALE) strcpy(wd_her,PlayerCharacter.load(x).name);
	else strcpy(wd_him,PlayerCharacter.load(x).name);
	strcpy(wd_them,PlayerCharacter.load(x).name);
}
