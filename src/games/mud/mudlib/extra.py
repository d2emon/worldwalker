...
if(brkword()!= -1)
{
	...
	if((PlayerCharacter.load(a).location_id!=curch))
	{
	...
	if(PlayerCharacter.load(mynum).helping_id!=-1)
	{
		sprintf(b,"\001c%s\001 has stopped helping you\n",globme);
		sendsys(PlayerCharacter.load(a).name,PlayerCharacter.load(a).name,-10011,curch,b);
		bprintf("Stopped helping %s\n",PlayerCharacter.load(PlayerCharacter.load(mynum).helping_id).name);
	}
	PlayerCharacter.load(mynum).helping_id = a
 ...
 valuecom()
    ...
    bprintf("%s : %d points\n",wordbuf,(tscale()*(Item.load(b).base_value))/5);
    return;
    }
 stacom()
    {
    ...
    bprintf("\nItem        :%s",Item.load(a).name);
if(Item.load(a).is_in_item) bprintf(       "\nContained in:%s",Item.load(Item.load(a).location_id).name);
else
{
    if(not Item.load(a).is_in_location)bprintf("\nHeld By     :%s",PlayerCharacter.load(Item.load(a).location_id).name);
    else
       {bprintf("\nPosition    :");
       showname(Item.load(a).location_id);
}
       }
    bprintf("\nState       :%d",state(a));
    bprintf("\nCarr_Flag   :%d", Item.load(a).location_flag);
    bprintf("\nSpare       :%d", not Item.load(a).is_destroyed);
    bprintf("\nMax State   :%d", Item.load(a).max_state);
    bprintf("\nBase Value  :%d", Item.load(a).base_value);
    bprintf("\n");
    }
 examcom()
    {
    ...
    switch(a)
       {
       case 144:
          if(Item.load(144).get_value(0)==0)
             {
             Item.load(144).set_value(0, 1)
             bprintf("You take a scroll from the tube.\n");
            Item.load(145).create()
            Item.load(145).set_location_held(mynum);
             return;
             }
          break;
       case 145:
          ...
          return;
       case 101:
          if(Item.load(101).get_value(0)==0)
             {
             bprintf("You take a key from one pocket\n");
            Item.load(101).set_value(0, 1)
             Item.load(107).create()
             Item.load(107).set_location_held(mynum);
             return;
             }
          break;
       case 7:
       ...
       case 8:
          if(state(7)!=0)
             {
             if((iscarrby(3+state(7),mynum))&&(Item.load(3+state(7)).get_flag(Item.FLAG_13)))
                {
 ...
 bprintf("Name      : %s\n",PlayerCharacter.load(b).name);
 bprintf("Level     : %d\n",PlayerCharacter.load(b).level);
 bprintf("Strength  : %d\n",PlayerCharacter.load(b).strength);
 bprintf("Sex       : %s\n",(PlayerCharacter.load(b).sex == PlayerCharacter.MALE)?"MALE":"FEMALE");
 bprintf("Location  : ");
 showname(PlayerCharacter.load(b).location_id);
 }
 wizlist()
 {
 extern long my_lev;
...
if((iscarrby(111,mynum))||(iscarrby(121,mynum))||(iscarrby(163,mynum)))
 ...
 while(cha<numobs)
 {
 if(!strcmp(Item.load(cha).name,wordbuf))
    {
    rnd=1;
if(my_lev>9999) bprintf("[%3d]",cha);
    bprintf("%16s - ",Item.load(cha).name);
    if((my_lev<10)&&(Item.load(cha).is_destroyed)) bprintf("Nowhere\n");
    else
       desrm(Item.load(cha).location_id, Item.load(cha).location_flag);
    }
 cha++;
 }
 cha=fpbn(wordbuf);
 ...
 {
 rnd++;
 bprintf("%s - ",PlayerCharacter.load(cha).name);
 desrm(PlayerCharacter.load(cha).location_id,0);
 }
 if(rnd) return;
