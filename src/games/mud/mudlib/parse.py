 ...
 doaction(n)
    ...
    switch(n)
       ...
       case 8:
          ...
          dumpitems();
          PlayerCharacter.load(mynum).kill();
          PlayerCharacter.load(mynum).remove()
          closeworld();
          curmode=0;curch=0;
          saveme();
          crapup("Goodbye");
...
dogocom(n)
 ...
 dodirn(n)
    ...
    if((iscarrby(32,mynum))&&(PlayerCharacter.load(25).channel_id==curch)&&(PlayerCharacter.load(25).exists))
       {
    ...
    if((newch>999)&&(newch<2000))
       ...
       if(state(drnum)!=0)
          {
	  if (strcmp(Item.load(drnum).name,"door")||isdark()||strlen(Item.load(drnum).get_description(state(drnum)))==0)
              {
       ...
       newch=Item.load(droff).location_id;
       }
    if(newch==-139)
       {
    ...
    if(n==2)
       {
         if(((i=fpbns("figure"))!=mynum)&&(i!=-1)&&(PlayerCharacter.load(i).channel_id==curch)&&!iswornby(101,mynum)&&!iswornby(102,mynum)&&!iswornby(103,mynum))
    	    {
    ...
    if(newch>=0)bprintf("You can't go that way\n");
       ...
       {
       sprintf(block,"%s%s%s%s%s%s%s%s%s%s","\001s",PlayerCharacter.load(mynum).name,"\001",globme," has gone ",exittxt[n]," ",out_ms,".","\n\001");
...
long zapped;
 ...
 long *blok;
    ...
    switch(blok[1])
       {
       case -9900:
          PlayerCharacter.load(i[0]).visible = i[1];break;
       case -666:
...
eorte()
{
    ...
    if(me_ivct) me_ivct--;
    if(me_ivct==1) PlayerCharacter.load(mynum).visible = 0;
    if(me_cal)
       {
       me_cal=0;
       calibme();
    ...
    if(in_fight)
    {
       if(PlayerCharacter.load(fighting).channel_id!=curch)
          {
          fighting= -1;
          in_fight=0;
          }
       if(!PlayerCharacter.load(fighting).exists)
          {
          fighting= -1;
          in_fight=0;
          }
...
long me_cal=0;
 ...
 lightning()
    ...
    sendsys(PlayerCharacter.load(vic).name,globme,-10001,PlayerCharacter.load(vic).channel_id,"");
    logging.info("%s zapped %s",globme,PlayerCharacter.load(vic).name);
 ...
 eatcom()
    ...
    switch(b)
       ...
       case 175:
          ...
          break;
       default:
          if(Item.load(b).get_flag(Item.FLAG_6))
             {
             destroy(b);
             bprintf("Ok....\n");
             my_str+=12;
 ...
 calibme()
    ...
    if(b!=my_lev)
       ...
       logging.info("%s to level %d",globme,b);
       disle3(b,my_sex);
       sprintf(sp,"\001p%s\001 is now level %d\n",globme,my_lev);
       sendsys(globme,globme,-10113,PlayerCharacter.load(mynum).channel_id,sp);
       if(b==10) bprintf("\001f%s\001",GWIZ);
       }
    PlayerCharacter.load(mynum).level = my_lev
    if(my_str>(30+10*my_lev)) my_str=30+10*my_lev;
    PlayerCharacter.load(mynum).strength = my_str
    PlayerCharacter.load(mynum).sex = my_sex
    PlayerCharacter.load(mynum).weapon_id = wpnheld
    }
 ...
 playcom()
    ...
    if(!Item.load(a).available_for(mynum))
       {
       bprintf("That isn't here\n");
       return;
       }
 ...
 tellcom()
    ...
    getreinput(blob);
    sendsys(PlayerCharacter.load(b).name,globme,-10004,curch,blob);
    }
 ...
 exorcom()
    ...
    logging.info("%s exorcised %s",globme,PlayerCharacter.load(x).name);
    dumpstuff(x,PlayerCharacter.load(x).channel_id);
    sendsys(PlayerCharacter.load(x).name,globme,-10010,curch,"");
    PlayerCharacter.load(x).remove()
    }
 ...
 dogive(ob,pl)
    ...
    extern long mynum;
    if((my_lev<10)&&(PlayerCharacter.load(pl).channel_id!=curch))
       {
    ...
    Item.load(ob).set_location_held(pl);
    sprintf(z,"\001p%s\001 gives you the %s\n",globme,Item.load(ob).name);
    sendsys(PlayerCharacter.load(pl).name,globme,-10011,curch,z);
    return;
    }
 ...
 stealcom()
    ...
    if((my_lev<10)&&(PlayerCharacter.load(c).channel_id!=curch))
       {
       bprintf("But they aren't here\n");
       return;
       }
    if(Item.load(a).is_worn)
       {
       bprintf("They are wearing that\n");
       return;
       }
    if(PlayerCharacter.load(c).weapon_id == a)
       {
    ...
    if(f<e)
       ...
       if(f&1){
       	 sendsys(PlayerCharacter.load(c).name,globme,-10011,curch,tb);
       	 if(c>15) woundmn(c,0);
       	}
...
bugcom()
{
	...
	extern char globme[];
	getreinput(x);
	logging.info("Bug by %s : %s",globme,x);
}
...
look_cmd()
{
	...
	if(!Item.load(a).get_flag(Item.FLAG_14))
	{
		bprintf("That isn't a container\n");
		return;
	}
	if((Item.load(a).get_flag(Item.FLAG_2))&&(state(a)!=0))
	{
		bprintf("It's closed!\n");
		return;
	}
...
{
        extern long curch;
	if((Item.load(186).location_id ==curch)&&(isdest(186)))
	{
		bprintf("You uncover a stone slab!\n");
		Item.load(186).create()
		return;
	}
...
emptycom()
{
	...
	while(b<numobs)
	{
		if(iscontin(b,a))
		{
			Item.load(b).set_location_held(mynum);
			bprintf("You empty the %s from the %s\n",Item.load(b).name,Item.load(a).name);
			sprintf(x,"drop %s",Item.load(b).name);
			gamecom(x);
			pbfr();
			openworld();
		}
