...
if(tty==4) btmscr();
strcpy(prmpt,"\r");
    if(PlayerCharacter.load(mynum).visible) strcat(prmpt,"(");
    ...
    if(PlayerCharacter.load(mynum).visible) strcat(prmpt,")");
    pbfr();
    if(PlayerCharacter.load(mynum).visible>9999) set_progname(0,"-csh");
    else
    sprintf(work,"   --}----- ABERMUD -----{--     Playing as %s",name);
    if(PlayerCharacter.load(mynum).visible==0) set_progname(0,work);
    sig_alon();
    key_input(prmpt,80);
    sig_aloff();
...
long rd_qd=0;
 ...
 if(my_lev<10000) PlayerCharacter.load(mynum).visible = 0;
    else PlayerCharacter.load(mynum).visible = 10000;
          PlayerCharacter.load(mynum).weapon_id = -1
          PlayerCharacter.load(mynum).flags = my_sex
...
extern long curch;
    ...
    ndie:unit=openworld();
    PlayerCharacter.load(mynum).channel_id = chan;
    lookin(chan);
    }
...
long mynum=0;
 ...
 char *name;
    ...
    while((f==0)&&(ct<maxu))
       {
       if (!PlayerCharacter.load(ct).exists) f=1;
       else
          ct++;
       }
    if(ct==maxu)
    ...
    PlayerCharacter.load(ct).name = name
    PlayerCharacter.load(ct).channel_id = curch
    PlayerCharacter.load(ct).event_id = -1
    PlayerCharacter.load(ct).level = 1
    PlayerCharacter.load(ct).visible = 0
    PlayerCharacter.load(ct).strength = -1
    PlayerCharacter.load(ct).weapon_id = -1
    PlayerCharacter.load(ct).sex = 0
    mynum=ct;
iamon=1;
    }
...
unit=openworld();
    dumpitems();
if(PlayerCharacter.load(mynum).visible<10000) {
...
long lasup=0;
 ...
 char *name;
    ...
    if(xp<10) goto noup;
    unit=openworld();
    PlayerCharacter.load(mynum).event_id = cms
    lasup=cms;
    noup:;
    }
 ...
 long cutoff;
    ...
    while(ct<16)
       {
       if((PlayerCharacter.load(ct).exists)&&(PlayerCharacter.load(ct).event_id < cutoff/2)&&(not PlayerCharacter.load(ct).is_absent))
          {
          sprintf(mess,"%s%s",PlayerCharacter.load(ct).name," has been timed out\n");
          broad(mess);
          dumpstuff(ct,PlayerCharacter.load(ct).channel_id);
          PlayerCharacter.load(ct).remove()
          }
       ct++;
       }
    }
...
extern char globme[];
extern long iamon;
if(fpbns(globme)!= -1) {loseme();logging.info("System Wrapup exorcised %s",globme);}
}
