 ...
 doaction(n)
    ...
    switch(n)
       ...
       case 8:
          ...
          pname(mynum)[0]=0;
          closeworld();
          talker.mode = talker.MODE_SPECIAL
          curch=0;
          saveme();
          crapup("Goodbye");
          break;
...
dogocom(n)
 ...
 dodirn(n)
    ...
    if((newch>999)&&(newch<2000))
       ...
       if(state(drnum)!=0)
          {
	  if (strcmp(oname(drnum),"door")||WeatherServices.get_is_dark(talker.channel)||strlen(olongt(drnum,state(drnum)))==0)
              {
...
"""
 ...
 convcom()
    {
    talker.conversation_mode = talker.CONVERSATION_MODE_SAY
 ...
 shellcom()
    ...
    talker.conversation_mode = talker.CONVERSATION_MODE_TSS
