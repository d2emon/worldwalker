...
((iscontin(c,loc))&&(mode==3)))
          ...
          e=1;
              f+=1+strlen(oname(c));
if(Parser.debug_mode){ f+=5;sprintf(x,"%d",c);sprintf(y,"{%-3s}",x);}
if(isdest(c)) f+=2;
...
if(isdest(c)) bprintf("(");
         bprintf("%s",oname(c));
         if(Parser.debug_mode) bprintf(y);
...
"""
 lispeople()
    ...
    while(a<48)
       ...
       if((!!strlen(pname(a)))&&(ploc(a)==curch)&&(BprintF.seeplayer(a, Talker, New1)))
          {
          b=1;
         bprintf("%s ",pname(a));
         if(Parser.debug_mode) bprintf("{%d}",a);
          disl4(plev(a),psex(a));
