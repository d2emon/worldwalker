def is_dark(user):
    def idk():
        ...
        :return:
        """
        # for c in game_objects:
        #     if c.object_id != 32 and not c.test_bit(13):
        #         continue
        #     if is_here(c):
        #         return False
        #     if c.carry_flag == 0 or c.carry_flag == 3:
        #         continue
        #     if Player.players[c.location].location != user.curch:
        #         continue
        #     return False
        return True
    ...
    if user.my_lev > 9:
        return False
    if user.curch == -1100 or user.curch == -1101:
        return False
    if user.curch <= -1113 and user.curch >= -1123:
        return idk()
    if user.curch < -399 or user.curch > -300:
        return False
    return idk()
